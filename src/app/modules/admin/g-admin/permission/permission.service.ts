...
import { AssetItem, Store, AssetType, Chat, PermissionProductDetailOSM, PermissionPagination, PermissionProduct, PermissionProductDetail, StoreType, AssetSize, Supplier, Division } from './permission.types';
...
export class PermissionService {
    // Private
    private _pagination: BehaviorSubject<PermissionPagination | null> = new BehaviorSubject(null);
    private _product: BehaviorSubject<PermissionProduct | null> = new BehaviorSubject(null);
    private _products: BehaviorSubject<PermissionProduct[] | null> = new BehaviorSubject(null);
    private _product_osm: BehaviorSubject<PermissionProduct | null> = new BehaviorSubject(null);
    private _products_osm: BehaviorSubject<PermissionProduct[] | null> = new BehaviorSubject(null);
     ...
     */
    get pagination$(): Observable<PermissionPagination> {
     ...
     * Getter for product
     */
    get product$(): Observable<PermissionProduct> {
     ...
     * Getter for products
     */
    get products$(): Observable<PermissionProduct[]> {
     ...
     * Getter for product
     */
    get product_osm$(): Observable<PermissionProduct> {
...
*/
    ...
    getProducts(page: number = 0,
        ...
        Observable<{ meta: PermissionPagination; data: PermissionProduct[] }> {
        ...
        return this._httpClient.get<{ meta: PermissionPagination; data: PermissionProduct[] }>(environment.API_URL + 'api/briefs', {
    ...
    getProductsND(page: number = 0,
        ...
        return this._httpClient.get<{ meta: PermissionPagination; data: PermissionProduct[] }>(environment.API_URL + 'api/brief-osms', {
            params: searchParams, headers: new HttpHeaders({ Authorization: `Bearer ${token}` })
        }).pipe(
            tap((response) => {
                this._pagination.next({
     ...
     * Get product by id
     */
    getProductById(id: string): Observable<PermissionProductDetail> {
        // this.getProducts();
    ...
    */
     ...
     * Create product
     */
    createProduct(): Observable<PermissionProduct> {
        return this.products$.pipe(
            take(1),
            switchMap(products => this._httpClient.post<PermissionProduct>('api/marketing/brief-plan/product', {}).pipe(
                map((newProduct) => {
     ...
     * @param product
     */
    updateProduct(id: string, product: PermissionProduct): Observable<PermissionProduct> {
        return this.products$.pipe(
            take(1),
            switchMap(products => this._httpClient.patch<PermissionProduct>('api/marketing/brief-plan/product', {
                id,
                product
            }).pipe(
                map((updatedProduct) => {
