...
import {
    ...
    saleOrderPagination, UserProduct, StoreType, AssetSize, Supplier, Division, DataSaleOrder
...
export class SaleOrderService {
    // Private
    private _pagination: BehaviorSubject<saleOrderPagination | null> = new BehaviorSubject(null);
     ...
     */
    get pagination$(): Observable<saleOrderPagination> {
  ...
  */
    ...
    getProducts(page: number = 0,
        ...
        Observable<{ meta: saleOrderPagination; data: UserProduct[] }> {
        ...
        return this._httpClient.get<{ meta: saleOrderPagination; data: UserProduct[] }>(environment.API_URL + 'api/briefs', {
    ...
    getProductsND(page: number = 0,
        ...
        return this._httpClient.get<{ meta: saleOrderPagination; data: UserProduct[] }>(environment.API_URL + 'api/brief-osms', {
            params: searchParams, headers: new HttpHeaders({ Authorization: `Bearer ${token}` })
        }).pipe(
            tap((response) => {
                this._pagination.next({
  ...
  */
    ...
    getdelivery(): Observable<any[]> {
        return this._httpClient.get<any[]>(environment.API_URL + 'api/get_deliveredBy').pipe(
            tap((user) => {
                this._materials.next(user);
            })
        );
    ...
    getbank(): Observable<any[]> {
        return this._httpClient.get<any[]>(environment.API_URL + 'api/get_bank').pipe(
            tap((user) => {
                this._materials.next(user);
            })
        );
    ...
    createsaleorder(saleorder: any): Observable<any> {
        return this._httpClient.post(environment.API_URL + 'api/sale_order', saleorder, this.httpOptionsFormdata).pipe(
            switchMap((response: any) => {
                // Return a new observable with the response
                return of(response);
            })
...
updatesaleorder(saleorderId: any,saleorder: any) : Observable<any> {
        return this._httpClient.put(environment.API_URL + 'api/sale_order/' + saleorderId, saleorder, this.httpOptionsFormdata).pipe(
          switchMap((response: any) => {
            // Return a new observable with the response
            return of(response);
          })
    ...
    getsaleorderbyId(saleorderId: any): Observable<any[]> {
        return this._httpClient.get<any[]>(environment.API_URL + 'api/sale_order/' + saleorderId).pipe(
            tap((meterial) => {
                this._materials.next(meterial);
            })
        );
