...
import {
  ...
  PositionPagination, PositionProduct, StoreType, AssetSize, Supplier, Division
} from './position.types';
...
export class PositionService {
  ...
  private _product: BehaviorSubject<PositionProduct | null> = new BehaviorSubject(null);
  private _products: BehaviorSubject<PositionProduct[] | null> = new BehaviorSubject(null);
  private _product_osm: BehaviorSubject<PositionProduct | null> = new BehaviorSubject(null);
  private _products_osm: BehaviorSubject<PositionProduct[] | null> = new BehaviorSubject(null);
...
*/
  ...
  getProducts(page: number = 0,
    ...
    return this._httpClient.get<{ meta: PositionPagination; data: PositionProduct[] }>(environment.API_URL + 'api/briefs', {
  ...
  getProductsND(page: number = 0,
    ...
    return this._httpClient.get<{ meta: PositionPagination; data: PositionProduct[] }>(environment.API_URL + 'api/brief-osms', {
      params: searchParams, headers: new HttpHeaders({ Authorization: `Bearer ${token}` })
    }).pipe(
      tap((response) => {
        this._pagination.next({
  ...
  */
  ...
  createProduct(): Observable<PositionProduct> {
    return this.products$.pipe(
      take(1),
      switchMap(products => this._httpClient.post<PositionProduct>('api/marketing/brief-plan/product', {}).pipe(
        map((newProduct) => {
   ...
   * @param product
   */
  updateProduct(id: string, product: PositionProduct): Observable<PositionProduct> {
    return this.products$.pipe(
      take(1),
      switchMap(products => this._httpClient.patch<PositionProduct>('api/marketing/brief-plan/product', {
        id,
        product
      }).pipe(
        map((updatedProduct) => {
...
*/
  ...
  createPosition(position: any): Observable<any> {
    return this._httpClient.post(environment.API_URL + 'api/position', position).pipe(
      switchMap((response: any) => {
        // Return a new observable with the response
        return of(response);
      })
  ...
  getPosition(): Observable<any> {
    return this._httpClient.get<any>(environment.API_URL + 'api/get_position').pipe(
      tap((meterial) => {
        this._materials.next(meterial);
      })
    );
  ...
  //* get position by id
  getPositionById(positionId: any): Observable<any[]> {
    return this._httpClient.get<any[]>(environment.API_URL + 'api/position/' + positionId).pipe(
      tap((meterial) => {
        this._materials.next(meterial);
      })
    );
  ...
  //   * update branch
  updatePosition(position: any, positionId): Observable<any> {
    return this._httpClient.put(environment.API_URL + 'api/position/' + positionId, position).pipe(
      switchMap((response: any) => {
        // Return a new observable with the response
        return of(response);
      })
  ...
  getPositionPage(dataTablesParameters: any): Observable<DataTablesResponse> {
    return this._httpClient.post(environment.API_URL + 'api/position_page', dataTablesParameters).pipe(
      switchMap((response: any) => {
